...
geometric_mean <- function(x = NULL, learn = FALSE, interactive = FALSE) {
  ...
  # Interactive mode - original code from interactive.geometricMean
  if (interactive) {
  ...
  # Learning mode - original code from explain.geometricMean
  if (learn) {
    x <- as.vector(x)
    producto <- 1
  ...
  # Simple calculation mode - original code from geometricMean_
  x <- as.vector(x)
  producto <- 1
  for(i in 1:length(x)) {
    producto <- producto * x[i]
